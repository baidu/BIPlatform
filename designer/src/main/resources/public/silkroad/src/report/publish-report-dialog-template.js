define(['template'], function (template) {
    function anonymous($data,$filename
        ...
        'use strict';
        $data=$data||{};
        var $utils=template.utils,$helpers=$utils.$helpers,$escape=$utils.$escape,url=$data.url,type=$data.type,content=$data.content,$out='';$out+='<div class="con-releaseBox">\r\n    <div class="con-head">\r\n        <a href="';
        $out+=$escape(url);
        $out+='" ';
        if(url!=='#'){
        $out+='target="_blank"';
        }
        $out+='><div class="con-hRead"></div></a>\r\n        ';
        if(type == 'POST'){
