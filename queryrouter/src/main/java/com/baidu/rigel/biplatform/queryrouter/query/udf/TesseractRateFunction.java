...
import com.baidu.rigel.biplatform.queryrouter.query.udf.condition.RateCondition;
...
/**
 * 类TesseractRateFunction.java的实现描述：TODO 类实现描述 
 * @author luowenlei 2015年12月9日 下午8:25:38
 */
abstract class TesseractRateFunction extends RateFunNode {
    ...
    Map<Condition, Set<String>> collectVariableCondition(RateCondition.RateType type) {
        ...
        Condition numeratorCondition = EmptyCondition.getInstance();
        // new RateCondition(true, type, variable.getVariableExp());
    ...
    protected void preSetNodeResult(CompileContext context) {
        ...
        variable.setResult(context.getVariablesResult().get(condition)
                .get(variable.getVariableExp()));
        
        VariableNode numeratorNodeVar = DeepcopyUtils.deepCopy(variable);
        numeratorNodeVar.setResult(context.getVariablesResult().get(EmptyCondition.getInstance())
                .get(variable.getVariableExp()));
